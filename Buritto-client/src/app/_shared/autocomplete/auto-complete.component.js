...
var AutoCompleteComponent = (function () {
    ...
    AutoCompleteComponent.prototype.setRecentLocation = function (data) {
        ...
        var el = parser.parseFromString(data.adr_address, "text/html");

        let locality = el.getElementsByClassName("locality")[0].outerText;
        let countryName = el.getElementsByClassName("country-name")[0].outerText;
        data.parsedAdress = `${locality}, ${countryName}`;

        data.adr_address = data.adr_address ? data.parsedAdress : data.formatted_address;
        data.active = false;
        this.selectedDataIndex = -1;
        this.locationInput = data.adr_address;
        if (this.settings.showRecentSearch) {
